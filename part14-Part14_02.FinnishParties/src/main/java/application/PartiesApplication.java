...
public class PartiesApplication extends Application {
    ...
    public HashMap<String, HashMap<Integer, Double>> readData(String fileName) {
        HashMap<String, HashMap<Integer, Double>> electionVoting = new HashMap<>(); 
            ...
            while (input.hasNext()) {
                ...
                for (int i = 1; i < parts.length; i++) {
                    if (!parts[i].equals("-")) {
                        ...
                        double precentage = Double.parseDouble(parts[i]);
                        dataValues.put(votingYear, precentage);
                    }
                }
                
                electionVoting.put(politicalParty, dataValues); 
